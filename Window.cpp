...
{
	{
		...
		if ((_depth_stencil_format == VK_FORMAT_D32_SFLOAT_S8_UINT) ||
			...
			(_depth_stencil_format == VK_FORMAT_D16_UNORM))
		{
	...
	//uint32_t memroy_index = UINT32_MAX;
	...
	for (uint32_t i = 0; i < gpu_memroy_properties.memoryTypeCount ; ++i)
	{
		if (image_memory_requirements.memoryTypeBits && (1 << i))
		{
			if ((gpu_memroy_properties.memoryTypes[i].propertyFlags && required_properties) == required_properties )
			{
				memroy_index = i;
				break;
			}
	...
	assert(memroy_index != UINT32_MAX);
	*/
